...
def render(st):
    ...
    if st.session_state.get("flight2_save_msg"):
        st.success(st.session_state.get("flight2_save_msg"))
    ...
    col_table, col_selects = st.columns([2,1])
    with col_table:
        ...
        st.markdown(
            f"""
            <style>
            #ladies_f2 div[data-testid='stDataEditor'] {{
                width:{FIXED_WIDTH}px !important;
                min-width:{FIXED_WIDTH}px !important;
                max-width:{FIXED_WIDTH}px !important;
            }}
            #ladies_f2 div[data-testid='stDataEditor'] table {{
                width:{FIXED_WIDTH}px !important;
            }}
            #ladies_f2 div[data-testid='stDataEditor'] tbody td, 
            #ladies_f2 div[data-testid='stDataEditor'] thead th {{
                padding:2px 4px !important;
                white-space:nowrap !important;
            }}
            </style>
            <div id='ladies_f2'></div>
            """,
            unsafe_allow_html=True,
        )
        ladies_edited = st.data_editor(
            ladies_df,
            key="flight2_ladies_editor",
            hide_index=True,
            width='content',
            column_config={
                "Spieler": {"width": "small"},
    ...
    if save_clicked:
        try:
            ...
            saved_lines = [f"Gespeicherte Scores (Datum {dkey}, Flight 2):"]
            # Scores aktualisieren (nur Flight 2)
            ...
            st.session_state["flight2_save_msg"] = "\n".join(saved_lines)
            st.rerun()
        except Exception as e:
